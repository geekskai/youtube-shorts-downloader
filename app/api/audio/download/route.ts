...
export async function GET(request: Request) {
  ...
  try {
    ...
    const link = await getAudioDownloadLink(videoId, quality)
    ...
    if (error instanceof ShortsApiError) {
      console.error("[api/audio/download]", error.code, error.message)
      return NextResponse.json(
    ...
    console.error("[api/audio/download]", message)
